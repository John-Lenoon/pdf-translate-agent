...
| 核心设计 | [`core-design.md`](core-design.md) | 模型、上下文、路由、渲染和失败语义 |
...
| 实施规则 | [`implementation.md`](implementation.md) | 编码约定、模块分层和升级边界 |
...
| Wayfinder issue tracker | [`agents/issue-tracker.md`](agents/issue-tracker.md) | 本地决策地图、ticket、阻塞关系和解析规则 |

## Reading order

新功能先读 `requirements.md`、`roadmap.md` 和 `architecture.md`，再按影响范围阅读 `core-design.md`、`workflow.md`、`database.md` 或 `api.md`。实现和验证分别遵循 `implementation.md`、`testing.md` 与 `git.md`。

## Version boundaries

- V1 的基线和验收以各正式契约中的 V1 小节为准。
- V2 的双模型和渲染行为以 `core-design.md` 为准；其他文档只保留必要的接口或数据引用，不复制完整规则。
